...
\subsection{1b}
Teorin bakom detta står på \bl{sid 388}.\\\\
%
...
%
%
Polerna kan vi enligt ekvation 3.15 på \bl{sid 92} räkna ut med $\det(sI-A) = 0$. Hur man räknar ut determinanter står på \mhb{sid 93}.
%
\begin{align*}
...
%
%
Enligt (11.15) på \bl{sid 425} så gäller att när vi ``uttrycker den önskade polplaceringen för det återkopplade systemet i form av det karakteristiska polynomet ($\alpha_c(s)$), deder det till att följande polynomidentitet ska uppfyllas: $\alpha_c(s) = \det(sI -A + BL_u)$''. Med andra ord, den karakteristiska ekvationen vi nyss räknade ut ska vara lika med $\det(sI -A + BL)$.
%
\begin{align*}
...
%
%
Vi ersätter $s$ med $i\omega$ (det vill säga går över från Laplacedomänen till Fourierdomänen).
%
%
\begin{align*}
...
Vi börjar med att räkna ut $|G(i\omega)|$. På \mhb{sid 61} finns några användbara räkneregler för absolutbelopp: $|z_1 z_2| = |z_1|\cdot |z_2|$ och $\left|\frac{z_1}{z_2}\right| = \frac{|z_1|}{|z_2|}$. Vi ser också på \mhb{sid 62} att $|e^{i\theta}| = 1$.
\begin{align*}
...
Vi räknar nu ut $\arg\{ G(i\omega) \}$. På \mhb{sid 62} finns några användbara räkneregler för argument: $\arg\{ z_1 z_2 \} = \arg \{z_1\} + \arg \{z_2\}$ och $\arg\{ \frac{z_1}{z_2} \} = \arg\{ z_1 \} - \arg\{ z_2 \}$. Vi ser också att $\arg \{e^{i\theta}\} = \theta$.
